...
public class CardSlidePanel extends ViewGroup implements View.OnClickListener {
    ...
    //拖拽工具类
    //这个跟原生的ViewDragHelper差不多，只是修改了Interpolator
    ...
    //水平距离+垂直距离,超过这个值, 则下一层view完成向上一层view的过渡
    ...
    public CardSlidePanel(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CardSlidePanel);

        itemMarginTop = (int) a.getDimension(R.styleable.CardSlidePanel_itemMarginTop, itemMarginTop);
        bottomMarginTop = (int) a.getDimension(R.styleable.CardSlidePanel_bottomMarginTop, bottomMarginTop);
        yOffsetStep = (int) a.getDimension(R.styleable.CardSlidePanel_yOffsetStep, yOffsetStep);
        // 滑动相关类
        dragHelper = ViewDragHelper.create(this, 10f, new DragHelperCallback());
        dragHelper.setEdgeTrackingEnabled(ViewDragHelper.EDGE_BOTTOM);
        a.recycle();

        ViewConfiguration configuration = ViewConfiguration.get(getContext());
        touchSlop = configuration.getScaledTouchSlop();
        moveDetector = new GestureDetectorCompat(context, new SimpleOnGestureListener() {
            @Override
            public boolean onScroll(MotionEvent e1, MotionEvent e2, float dx, float dy) {
                // 拖动了，touch不往下传递
                return Math.abs(dy) + Math.abs(dx) > touchSlop;
            }
        });
    ...
    protected void onFinishInflate() {
        ...
        for (int i = num - 1; i >= 0; i--) {
            ...
            if (childView.getId() == R.id.linear_bottom) {
                ...
                viewItem.setTag(i + 1);
                viewItem.maskView.setOnClickListener(this);
                viewList.add(viewItem);
            }
        }

        findViewById(R.id.button_nolike).setOnClickListener(this);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.view_mask:
                ...
                break;
            case R.id.button_nolike:
                CardSlidePanel.this.vanishOnBtnClick(VANISH_TYPE_LEFT);
                break;
            case R.id.button_like:
                CardSlidePanel.this.vanishOnBtnClick(VANISH_TYPE_RIGHT);
                break;
    ...
    private void animToSide(CardItemView changedView, int xvel, int yvel) {
        ...
        if (finalX == initCenterViewX) {
            ...
            if (dragHelper.smoothSlideViewTo(changedView, finalX, finalY)) {
    ...
    private void vanishOnBtnClick(int type) {
        ...
        if (finalX != 0) {
            ...
            if (dragHelper.smoothSlideViewTo(animateView, finalX, initCenterViewY + allHeight / 2)) {
                ViewCompat.postInvalidateOnAnimation(this);
            }
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (dragHelper.getViewDragState() == ViewDragHelper.STATE_SETTLING) {
                dragHelper.abort();
            }
            orderViewStack();

            // 保存初次按下时arrowFlagView的Y坐标
            // action_down时就让dragHelper开始工作，否则有时候导致异常
    ...
    public boolean onTouchEvent(MotionEvent e) {
        try {
            // 统一交给dragHelper处理，由DragHelperCallback实现拖动效果
